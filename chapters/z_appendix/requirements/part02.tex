\section{Anforderungen an das Erscheinungsbild} \label{sec:anforderungsspezifikation:erscheinungsbild}
...
\newline \textbf{Testkriterium}: Es können keine inkonsistenten Daten gefunden werden und es existiert ein Mechanismus zum Erhalten der Datenqualität.
\newline \textbf{Priorität}: 2
...
\textbf{Beschreibung}: Aus dem aktuellen BI-System sollen bis zu 500 GB Daten in die Cloud-Lösung übertragen werden können.
...
\subsection[Hinzufügen/Entfernen von Quellsystemen]{Es sollen jederzeit weitere Quellsysteme hinzugefügt/entfernt werden können} \label{sec:anforderungsspezifikation:QuellsystemeÄndern}
...
\newline \textbf{Testkriterium}: Das hinzufügen und entfernen von Quellsystemen ist jederzeit möglich.
\newline \textbf{Priorität}: 2
...
\section{Anforderungen an die Wartbarkeit} \label{sec:anforderungsspezifikation:wartbarkeit}

\subsection{Protokollierung zum Identifizieren von Fehlerquellen} \label{sec:anforderungsspezifikation:fehlerquellenIdentifizieren}
\textbf{Beschreibung}: Aufgetretene Fehler sollen nachvollziehbar sein, indem sie in einem Protokoll festgehalten werden. Die Fehlerursache soll damit möglichst einfach zu identifizieren sein.
...
\subsection[Authentifizierung mit Active Directory]{Authentifizierung mit dem Active Directory der Software AG} \label{sec:anforderungsspezifikation:SAG_AD}
...
\subsection{Effizientes löschen aller Kundendaten auf Anfrage} \label{sec:anforderungsspezifikation:löschenKundendaten}
\textbf{Beschreibung}: Es muss möglich sein, alle Daten eines bestimmten Kunden effizient aus dem System zu entfernen.
\newline \textbf{Begründung}: Laut DSGVO Art. 17 verfügen Personen über das \textit{Recht auf Vergessenwerden}. Das bedeutet, dass auf Anfrage alle Personendaten gelöscht werden müssen.
...
\subsection[Individuelle Auswertungen mit eingeschränkten Zugriff]{Zugriff auf bestimmte Personen- und Leistungsdaten nur über Betriebsrat möglich} \label{sec:anforderungsspezifikation:ZugriffMitBetriebsrat}
...
\newline \textbf{Begründung}: Die Auswertung von individuelle Leistungen wird gewünscht, erfordert aber die explizite Zustimmung des Betriebsrates.
\newline \textbf{Testkriterium}: Auf bestimmte Daten und Auswertungen ist der Zugriff nur möglich, wenn dies explizit für die Person vom Betriebsrat freigeschaltet wurde.
\newline \textbf{Priorität}: 2
...
\newline \textbf{Testkriterium}: Alle verwendeten Dienste verschlüsseln die Daten in Ruhe und während der Übertragung.
\newline \textbf{Priorität}: 2
